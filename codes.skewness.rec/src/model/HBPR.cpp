...
void HBPR::Train(int sample_times, int walk_steps, double alpha, int workers, double mu){
        ...
        while (count<jobs)
        {
            ...
            for (int w=1; w<=walk_steps; w++)
            {
                ...
                while (pnet.field[nid].fields[0]!=pnet.field[cid1].fields[0])
                    nid = pnet.NegativeSample();
                pnet.UpdateFBPRPair(w_vertex, w_vertex, vid, cid1, nid, dim, _alpha/w, margin/w, 0.0);
                margin -= 3.0;
            }
