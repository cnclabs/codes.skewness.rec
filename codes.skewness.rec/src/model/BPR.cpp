...
void BPR::LoadEdgeList(string filename, bool undirect) {
    pnet.LoadEdgeList(filename, undirect);
}

void BPR::SaveWeights(string model_name){
...
void BPR::Train(int sample_times, int negative_samples, double alpha, double reg, double margin, int workers){
    ...
    for (int worker=0; worker<workers; ++worker)
    {
        ...
        while (count<jobs)
        {
            ...
            pnet.UpdateBPRPair(w_vertex, w_vertex, v1, v2, v3, dim, reg, margin, _alpha);
            //pnet.UpdateSBPRPair(w_vertex, w_vertex, v1, v2, v2, dim, reg, _alpha);
